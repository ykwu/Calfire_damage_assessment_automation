...
URL = 'https://services1.arcgis.com/jUJYIo9tSA7EHvfZ/ArcGIS/rest/services/Camp2018_DINS_Public_View_Pictures/FeatureServer/0/query'
GEOCODE_URL = 'http://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/findAddressCandidates'
GEOCODE_MIN_SCORE = 80
DATA_CSV = 'damage1126.csv'
cityName = 'PARADISE'
...
def geocode(address):
    ...
    # check if this is in city
    
    CityFilter1 = address.upper().find(cityName)
    #print(CityFilter1)
    ...
    try:
        ...
        return best_candidate['extent'], best_candidate['location'], CityFilter1
...
def get_status(extent, location, cityfilter1):
    ...
    if cityfilter1 > 0:
        ...
        raise Uncertain('out of city limit: ' + cityName)
...
if __name__ == '__main__':
    ...
    with open('damage_outcome.csv', 'w') as f:
        ...
        for i in get_addresses():
            ...
            try:
                extent, location, cityFilter = geocode(address)
                status = get_status(extent, location, cityFilter)
